...
class Grafo ():
    ...
    def adicionar_vertice (self, dest):
        self.way = self.way + (dest,)

    def vertices (self):
        return self.way

    def adicionar_arco (self,dest):
        self.arc = self.arc + (dest,)

    def arcos (self, vertice):
        listArc = tuple ()
        for k in self.arc:
            if (vertice in k.vertices):
                listArc = listArc + (k,)
        return listArc
    ...
    def adjacencias (self, vertice):
        ...
        for k in self.arc:
            if (vertice in k.vertices):
                listAdja = listAdja + (k.oposto(vertice),)
        return listAdja

    def caminho (self, origem, destino):
        listCaminho = []
        if (origem == destino):
            listCaminho.append(origem)
        elif (self.arcos (origem) == tuple ()):
            return listCaminho
        else:
            verticeVez = origem
            while (verticeVez is not destino):
                for k in self.arcos(verticeVez):
                    if (k.oposto(verticeVez) not in listCaminho):
                        listCaminho.append(verticeVez)
                        verticeVez = k.oposto(verticeVez)
                        break
            listCaminho.append(verticeVez)
            return listCaminho
    ...
    def calcular_melhores_caminhos_partindo_de(self,vertice):
        ...
        while(len(self.vertices()) > len(visitados)):
            for k in self.arcos(verticeVez):
                if k.oposto(verticeVez) not in visitados:
                    if k.oposto(verticeVez) not in distancia.keys():
                        distancia[k.oposto(verticeVez)] = distancia [verticeVez] + k.valor
                    else:
                        if distancia[k.oposto(verticeVez)] > distancia [verticeVez] + k.valor:
                            distancia[k.oposto(verticeVez)] = distancia [verticeVez] + k.valor
            point = None
            small = None
            for k, i in distancia.items():
            ...
            value = 0
            for k in self.arcos(verticeVez):
                if k.oposto(verticeVez) in visitados:
                    if small is None:
                        small = distancia[k.oposto(verticeVez)] + k.valor
                    if distancia[k.oposto(verticeVez)] + k.valor <= small:
                        small = distancia[k.oposto(verticeVez)] + k.valor
                        point = k.oposto(verticeVez)
                        value = k.valor
            caminho[verticeVez] = caminho [point] + [value,verticeVez]

        for k, i in distancia.items ():
            listBestWay[k] = tuple ()
            listBestWay[k] = listBestWay[k] + (i,caminho [k])
