...
class Arvore:
    ...
    def __iter__(self):
        ...
        if self.raiz is not None:
            ...
            pilha.append (self.raiz.irmao_direito)
            pilha.append (self.raiz.filho_esquerdo)
            while len (pilha) > 0:
                tirar = pilha.pop(-1)
                if tirar is not None:
                    yield tirar.valor
                    pilha.append(tirar.irmao_direito)
                    pilha.append(tirar.filho_esquerdo)
    
    '''O(N) para tempo e espaço, para tempo porque percorre a pilha para tirar os elementos e colocá-los e para memória
    porque aumenta e diminui a pilha conforme adiciona e tira elementos'''
